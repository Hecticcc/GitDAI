  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      try {
        ...
        const { projectId, serviceId, environmentId, files, entrypoint } = deploymentData;
        ...
        debug('Deployment Variables', { projectId, serviceId, environmentId, files });

        // Format files for Railway's REST API
        const formattedFiles = {};
        for (const [path, content] of Object.entries(files)) {
          formattedFiles[path] = Buffer.from(content).toString('base64');
        }
